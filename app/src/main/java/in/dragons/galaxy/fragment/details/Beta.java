...
public class Beta extends AbstractHelper {
    ...
    @Override
    public void draw() {
        if (PreferenceManager.getDefaultSharedPreferences(detailsFragment.getActivity())
        ...
        if (!app.isInstalled()
                || !app.isTestingProgramAvailable()
                || PreferenceManager.getDefaultSharedPreferences(detailsFragment.getActivity())
                .getBoolean(PlayStoreApiAuthenticator.PREFERENCE_APP_PROVIDED_EMAIL, false)
                ) {
            return;
        }
        ...
        detailsFragment.getActivity().findViewById(R.id.beta_feedback)
                .setVisibility(app.isTestingProgramOptedIn() ? View.VISIBLE : View.GONE);

        detailsFragment.getActivity().findViewById(R.id.beta_subscribe_button)
                .setOnClickListener(new BetaOnClickListener(ViewUtils
                        .findViewById(detailsFragment.getActivity(), R.id.beta_message), app));

        detailsFragment.getActivity().findViewById(R.id.beta_submit_button)
                .setOnClickListener(v -> initBetaTask(new BetaFeedbackSubmitTask()).execute());

        detailsFragment.getActivity().findViewById(R.id.beta_delete_button).setOnClickListener(v ->
                initBetaTask(new BetaFeedbackDeleteTask()).execute());

        if (null != app.getUserReview() && !TextUtils.isEmpty(app.getUserReview().getComment())) {
            ((EditText) detailsFragment.getActivity().findViewById(R.id.beta_comment))
    ...
    private BetaFeedbackTask initBetaTask(BetaFeedbackTask task) {
        ...
        task.setEditText(ViewUtils.findViewById(detailsFragment.getActivity(), R.id.beta_comment));
    ...
    static class BetaOnClickListener implements View.OnClickListener {
        ...
        BetaOnClickListener(TextView messageView, App app) {
    ...
    static abstract private class BetaFeedbackTask extends PlayStorePayloadTask<Void> {
        ...
        void setDeleteButton(View deleteButton) {
